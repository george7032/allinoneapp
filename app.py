...
try:
    ...
    def get_anthropic_key():
        # Streamlit Cloud: use st.secrets; local runs can use environment variables.
        try:
            return st.secrets.get("ANTHROPIC_API_KEY") or os.environ.get("ANTHROPIC_API_KEY")
        except Exception:
            return os.environ.get("ANTHROPIC_API_KEY")

    GENAI_AVAILABLE = bool(get_anthropic_key())
except ImportError:
...
def main():
    ...
    with st.expander("Full data profile"):
        ...
        if not profile["correlation"].empty:
            st.dataframe(profile["correlation"], use_container_width=True)
    ...
    if problem["paradigm"] == "classification" and target_col is not None:
        if df[target_col].dropna().nunique() < 2:
            st.warning("The selected target contains fewer than two usable classes. Choose another target.")
            return
    ...
    feature_cols = [
        ...
        if c != target_col and c not in profile["datetime_cols"]
    ]

    if not feature_cols:
        st.warning("No usable feature columns remain after excluding the target and datetime columns.")
        return
    ...
    if paradigm == "classification":
        ...
        if not profile["numeric_cols"]:
            st.warning("No numeric column is available for forecasting.")
            return
